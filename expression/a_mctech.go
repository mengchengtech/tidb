...
func (b *builtinMCTechGetFullSQLSig) evalString(row chunk.Row) (sql string, isNull bool, err error) {
	...
	switch len(b.args) {
	case 2: // args[1] => `txId`
		if txID, isNull, err = evalUint64(b.args[1], row); isNull || err != nil {
			return "", isNull, err
		}
	case 3: // args[1], args[2] => `connId`, `txId` 或者 args[1] => `txId`, args[2] => `group`
		var arg1Value uint64
		if arg1Value, isNull, err = evalUint64(b.args[1], row); err != nil || isNull {
			return "", true, err
		}
		arg2 := b.args[2]
		...
		case types.ETString:
			...
			if arg2Value, isNull, err = arg2.EvalString(b.ctx, row); isNull || err != nil {
				return "", true, err
			}
		...
		case types.ETInt:
			connID = arg1Value
			if txID, isNull, err = evalUint64(arg2, row); isNull || err != nil {
				return "", true, err
			}
		}
	case 4: // args[1], args[2], args[3] => `connId`, `txId`, `group`
		if connID, isNull, err = evalUint64(b.args[1], row); err != nil || isNull {
			fmt.Println("[4] args1 null")
			return "", true, err
		}
		if txID, isNull, err = evalUint64(b.args[2], row); err != nil || isNull {
			fmt.Println("[4] args2 null")
			return "", true, err
		}
		if group, isNull, err = b.args[3].EvalString(b.ctx, row); err != nil || isNull {
...
func evalUint64(arg Expression, row chunk.Row) (val uint64, isNull bool, err error) {
	var v types.Datum
	if v, err = arg.Eval(row); err != nil || v.IsNull() {
		return 0, true, err
	}
