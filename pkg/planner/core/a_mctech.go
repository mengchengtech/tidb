...
func (e *Execute) getExtensionParams(ctx context.Context) ([]ast.ParamMarkerExpr, int) {
	stmt := e.PrepStmt
	index := slices.IndexFunc(stmt.Params, func(p ast.ParamMarkerExpr) bool {
	...
	return stmt.Params[index:], index
}
...
// MCTechLargeLogExtractor is used to extract some predicates of `large_sql_query`
type MCTechLargeLogExtractor struct {
	extractHelper
...
func (e *MCTechLargeLogExtractor) Extract(
	ctx PlanContext,
...
func (e *MCTechLargeLogExtractor) explainInfo(p *PhysicalMemTable) string {
	if e.SkipRequest {
		return "skip_request: true"
	}
	if !e.Enable {
		return fmt.Sprintf("only search in the current '%v' file", config.GetMCTechConfig().Metrics.LargeLog.Filename)
	}
...
func (e *MCTechLargeLogExtractor) setTimeRange(start, end int64) {
	const defaultLargeLogDuration = 24 * time.Hour
	...
	if start == 0 {
		startTime = endTime.Add(-defaultLargeLogDuration)
	}
	if end == 0 {
		endTime = startTime.Add(defaultLargeLogDuration)
	}
	timeRange := &TimeRange{
		StartTime: startTime,
		EndTime:   endTime,
...
func (e *MCTechLargeLogExtractor) buildTimeRangeFromKeyRange(keyRanges []*coprocessor.KeyRange) error {
...
func (e *MCTechLargeLogExtractor) decodeBytesToTime(bs []byte) (int64, error) {
...
func (e *MCTechLargeLogExtractor) decodeToTime(handle kv.Handle) (int64, error) {
