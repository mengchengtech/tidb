...
func (m *MCTech) renderShowFullSQLResultsetRows() (err error) {
	...
	runAt, err = expression.GetTimeValue(m.SCtx().GetExprCtx(), opt.RunAt, mysql.TypeDatetime, mysql.MaxDatetimeWidthWithFsp, time.Local)
	if err != nil {
...
func (m *MCTech) renderShowDWIndexResultsetRows() (err error) {
	...
	if sctx, err = AsSctx(m.SCtx()); err != nil {
		return err
	}
