...
func NewTiFlashTableRules(tbl *model.TableInfo, bundle *placement.Bundle) ([]*pdhttp.Rule, error) {
...
func NewTiFlashPartitionRule(partID int64, tbl *model.TableInfo, bundle *placement.Bundle) (*pdhttp.Rule, error) {
	if tbl.TiFlashReplica == nil {
		// 跳过不存在tiflash的表
		return nil, nil
	}
...
func PutTiFlashRulesWithDefaultRetry(ctx context.Context, rules []*pdhttp.Rule) (err error) {
...
func PutTiFlashRulesWithRetry(ctx context.Context, rules []*pdhttp.Rule, maxRetry int, interval time.Duration) (err error) {
	if maxRetry < 0 {
		maxRetry = 0
	}
...
func PutTiFlashRules(ctx context.Context, rules []*pdhttp.Rule) error {
...
var roleToPriorities = map[pdhttp.PeerRoleType]uint8{
	pdhttp.Learner:  0,
	pdhttp.Follower: 1,
	pdhttp.Voter:    2,
	pdhttp.Leader:   3,
...
func attachTiflshRuleFromBundle(id int64, rule *pdhttp.Rule, bundle *placement.Bundle) error {
	// 获取rule的优先级 Learner > Follower > Voter > Leader
	var (
		seletedRule *pdhttp.Rule                 // 选中的放置规则
