...
func (cc *clientConn) onExtensionPrepareExecuteEnd(stmt PreparedStatement, sql string, err error) {
	prepared, errNotFound := cc.ctx.GetSessionVars().GetPreparedStmtByID(uint32(stmt.ID()))
