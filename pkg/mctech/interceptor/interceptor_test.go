...
func TestFullSQLLogDbCheckerError(t *testing.T) {
	...
	defer func() {
		failpoint.Disable("github.com/pingcap/tidb/pkg/config/GetMCTechConfig")
		failpoint.Disable("github.com/pingcap/tidb/pkg/mctech/preps/DbCheckError")
	}()
	store := testkit.CreateMockStore(t)
	tk := initDbAndData(t, store)
	failpoint.Enable("github.com/pingcap/tidb/pkg/mctech/preps/DbCheckError", mock.M(t, "true"))
