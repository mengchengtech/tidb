...
// InfoForTest get info for test
func (g *GlobalValueInfo) InfoForTest() map[string]any {
...
type Comments interface {
	...
	Package() PackageComment // 执行sql所属的依赖包名称（公共包中执行的sql）Package
	InfoForTest() map[string]any
	String() string
}
...
// InfoForTest get info for test
func (ti *tenantValueInfo) InfoForTest() map[string]any {
...
// InfoForTest get info for test
func (r *prepareResult) InfoForTest() map[string]any {
	info := map[string]any{}
	if len(r.params) > 0 {
		info["params"] = maps.Clone(r.params)
	}
	info["comments"] = r.comments.InfoForTest()
	if len(r.dbPrefix) > 0 {
		info["prefix"] = r.dbPrefix
	}
	info["tenant"] = r.tenant.InfoForTest()
	if r.globalInfo.Global {
		info["global"] = r.globalInfo.InfoForTest()
	}
	return info
}
...
func (d *baseContext) InfoForTest() map[string]any {
	return d.prepareResult.InfoForTest()
}
