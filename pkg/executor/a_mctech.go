...
// ParseLargeLogBatchSize is the batch size of large-log lines for a worker to parse, exported for testing.
var ParseLargeLogBatchSize = 64

// mctechLargeLogRetriever is used to read large log log data.
type mctechLargeLogRetriever struct {
	table                 *model.TableInfo
	outputCols            []*model.ColumnInfo
	initialized           bool
	extractor             *plannercore.MCTechLargeLogExtractor
	files                 []logFile
	fileIdx               int
	fileLine              int
	checker               *mctechLargeLogChecker
	columnValueFactoryMap map[string]mctechLargeLogColumnValueFactory
	instanceFactory       func([]types.Datum)

	taskList      chan mctechLargeLogTask
	stats         *mctechLargeLogRuntimeStats
...
func (e *mctechLargeLogRetriever) retrieve(ctx context.Context, sctx sessionctx.Context) ([][]types.Datum, error) {
	if !e.initialized {
	...
	return e.dataForLargeLog(ctx)
}

func (e *mctechLargeLogRetriever) initialize(ctx context.Context, sctx sessionctx.Context) error {
	var err error
	...
	e.columnValueFactoryMap = make(map[string]mctechLargeLogColumnValueFactory, len(e.outputCols))
	for idx, col := range e.outputCols {
		...
		factory, err := getLargeLogColumnValueFactoryByName(sctx, col.Name.O, idx)
		if err != nil {
			return err
		}
		if factory == nil {
			panic(fmt.Sprintf("should never happen, should register new column %v into getLargeLogColumnValueFactoryByName function", col.Name.O))
		}
		e.columnValueFactoryMap[col.Name.O] = factory
	}
	// initialize checker.
	e.checker = &mctechLargeLogChecker{
		hasProcessPriv: hasProcessPriv,
		user:           sctx.GetSessionVars().User,
	}
	e.stats = &mctechLargeLogRuntimeStats{}
	if e.extractor != nil {
...
func (e *mctechLargeLogRetriever) close() error {
	for _, f := range e.files {
		err := f.file.Close()
		if err != nil {
			logutil.BgLogger().Error("close large log file failed.", zap.Error(err))
		}
	}
	if e.cancel != nil {
		e.cancel()
...
type parsedLargeLog struct {
	rows [][]types.Datum
	err  error
}

func (e *mctechLargeLogRetriever) getNextFile() *logFile {
...
func (e *mctechLargeLogRetriever) getPreviousReader() (*bufio.Reader, error) {
	fileIdx := e.fileIdx
...
func (e *mctechLargeLogRetriever) getNextReader() (*bufio.Reader, error) {
	file := e.getNextFile()
	if file == nil {
		return nil, nil
	}
...
func (e *mctechLargeLogRetriever) parseDataForLargeLog(ctx context.Context, sctx sessionctx.Context) {
	defer e.wg.Done()
	...
	e.parseLargeLog(ctx, sctx, reader, ParseLargeLogBatchSize)
}

func (e *mctechLargeLogRetriever) dataForLargeLog(ctx context.Context) ([][]types.Datum, error) {
	var (
		task mctechLargeLogTask
		ok   bool
	)
...
type mctechLargeLogChecker struct {
	// Below fields is used to check privilege.
	hasProcessPriv bool
	user           *auth.UserIdentity
	// Below fields is used to check large log time valid.
	enableTimeCheck bool
	timeRanges      []*timeRange
}

func (sc *mctechLargeLogChecker) hasPrivilege(userName string) bool {
	return sc.hasProcessPriv || sc.user == nil || userName == sc.user.Username
}

func (sc *mctechLargeLogChecker) isTimeValid(t types.Time) bool {
...
type mctechLargeLogTask struct {
	resultCh chan parsedLargeLog
}

type mctechLargeLogBlock []string

func (e *mctechLargeLogRetriever) getBatchLog(ctx context.Context, reader *bufio.Reader, offset *offset, num int) ([][]string, error) {
	var line string
	...
	for i := 0; i < num; i++ {
		for {
			...
			log = append(log, line)
			if strings.HasSuffix(line, variable.MCLargeLogSQLSuffixStr) {
				if strings.HasPrefix(line, "use") || strings.HasPrefix(line, variable.MCLargeLogRowPrefixStr) {
					continue
				}
				break
			}
...
func (e *mctechLargeLogRetriever) getBatchLogForReversedScan(ctx context.Context, reader *bufio.Reader, offset *offset, num int) ([][]string, error) {
	...
	var line string
	var queries []mctechLargeLogBlock
	var log []string
	var err error
	hasStartFlag := false
	...
	for {
		...
		if err != nil {
			if err == io.EOF {
				if len(log) == 0 {
					decomposedLargeLogTasks := decomposeToLargeLogTasks(queries, num)
					offset.length = len(decomposedLargeLogTasks)
					return decomposedLargeLogTasks, nil
				}
				e.fileLine = 0
				reader, err = e.getPreviousReader()
				if reader == nil || err != nil {
					return decomposeToLargeLogTasks(queries, num), nil
				}
		...
		line = string(hack.String(lineByte))
		if !hasStartFlag && strings.HasPrefix(line, variable.MCLargeLogStartPrefixStr) {
			hasStartFlag = true
		}
		if hasStartFlag {
			log = append(log, line)
	...
	return decomposeToLargeLogTasks(queries, num), err
}

func decomposeToLargeLogTasks(logs []mctechLargeLogBlock, num int) [][]string {
	if len(logs) == 0 {
		return nil
	}
	...
	decomposedLargeLogTasks := make([][]string, 0)
	log := make([]string, 0, num*len(logs[0]))
	for i := range logs {
		log = append(log, logs[i]...)
		if i > 0 && i%num == 0 {
			decomposedLargeLogTasks = append(decomposedLargeLogTasks, log)
	...
	return decomposedLargeLogTasks
}

func (e *mctechLargeLogRetriever) parseLargeLog(ctx context.Context, sctx sessionctx.Context, reader *bufio.Reader, logNum int) {
	...
	for {
		...
		if err != nil {
			t := mctechLargeLogTask{}
			t.resultCh = make(chan parsedLargeLog, 1)
			select {
			case <-ctx.Done():
				return
			case e.taskList <- t:
			}
			e.sendParsedLargeLogCh(t, parsedLargeLog{nil, err})
		}
		...
		failpoint.Inject("mockReadLargeLogLarge", func(val failpoint.Value) {
			if val.(bool) {
		...
		for i := range logs {
			...
			go func() {
				...
				e.sendParsedLargeLogCh(t, parsedLargeLog{result, err})
				<-ch
			}()
...
func (e *mctechLargeLogRetriever) sendParsedLargeLogCh(t mctechLargeLogTask, re parsedLargeLog) {
	select {
	case t.resultCh <- re:
	default:
		return
...
func (e *mctechLargeLogRetriever) parseLog(ctx context.Context, sctx sessionctx.Context, log []string, offset offset) (data [][]types.Datum, err error) {
	start := time.Now()
	...
	defer func() {
		if r := recover(); r != nil {
			...
			buf = buf[:stackSize]
			logutil.BgLogger().Warn("large query parse large log panic", zap.Error(err), zap.String("stack", string(buf)))
		}
		if e.stats != nil {
	...
	failpoint.Inject("errorMockParseLargeLogPanic", func(val failpoint.Value) {
		if val.(bool) {
			panic("panic test")
		}
	})
	...
	for index, line := range log {
		...
		if !startFlag && strings.HasPrefix(line, variable.MCLargeLogStartPrefixStr) {
			row = make([]types.Datum, len(e.outputCols))
			user = ""
			valid := e.setColumnValue(sctx, row, tz, variable.MCLargeLogTimeStr, line[len(variable.MCLargeLogStartPrefixStr):], e.checker, fileLine)
			if valid {
				startFlag = true
			}
			continue
		...
		if startFlag {
			if strings.HasPrefix(line, variable.MCLargeLogRowPrefixStr) {
				line = line[len(variable.MCLargeLogRowPrefixStr):]
				valid := true
				if strings.HasPrefix(line, variable.MCLargeLogUserAndHostStr+variable.MCLargeLogSpaceMarkStr) {
					value := line[len(variable.MCLargeLogUserAndHostStr+variable.MCLargeLogSpaceMarkStr):]
					...
					valid = e.setColumnValue(sctx, row, tz, variable.MCLargeLogUserStr, user, e.checker, fileLine)
					if !valid {
						startFlag = false
						continue
					}
					host := parseUserOrHostValue(fields[1])
					valid = e.setColumnValue(sctx, row, tz, variable.MCLargeLogHostStr, host, e.checker, fileLine)
				} else {
			...
			} else if strings.HasSuffix(line, variable.MCLargeLogSQLSuffixStr) {
				...
				if strings.HasPrefix(line, variable.MCLargeLogGzipPrefixStr) {
					// 解压缩
				...
				_ = e.setColumnValue(sctx, row, tz, variable.MCLargeLogSQLStr, string(hack.Slice(line)), e.checker, fileLine)
...
func (e *mctechLargeLogRetriever) setColumnValue(sctx sessionctx.Context, row []types.Datum, tz *time.Location, field, value string, checker *mctechLargeLogChecker, lineNum int) bool {
...
func (e *mctechLargeLogRetriever) setDefaultValue(row []types.Datum) {
	for i := range row {
		if !row[i].IsNull() {
			continue
		}
...
func (e *mctechLargeLogRetriever) getAllFiles(ctx context.Context, sctx sessionctx.Context, logFilePath string) ([]logFile, error) {
	totalFileNum := 0
	if e.stats != nil {
		startTime := time.Now()
		defer func() {
...
func (e *mctechLargeLogRetriever) getFileStartTime(ctx context.Context, file *os.File, compressed bool) (time.Time, error) {
	var t time.Time
	_, err := file.Seek(0, io.SeekStart)
	if err != nil {
		return t, err
	...
	for {
		...
		if strings.HasPrefix(line, variable.MCLargeLogStartPrefixStr) {
			return ParseTime(line[len(variable.MCLargeLogStartPrefixStr):])
		}
		maxNum--
		if maxNum <= 0 {
			break
...
func (e *mctechLargeLogRetriever) getRuntimeStats() execdetails.RuntimeStats {
	return e.stats
}

type mctechLargeLogRuntimeStats struct {
	totalFileNum int
	readFileNum  int
...
func (s *mctechLargeLogRuntimeStats) String() string {
...
func (s *mctechLargeLogRuntimeStats) Merge(rs execdetails.RuntimeStats) {
	tmp, ok := rs.(*mctechLargeLogRuntimeStats)
	if !ok {
		return
	}
...
func (s *mctechLargeLogRuntimeStats) Clone() execdetails.RuntimeStats {
	newRs := *s
	return &newRs
}

// Tp implements the RuntimeStats interface.
func (s *mctechLargeLogRuntimeStats) Tp() int {
	return execdetails.TpMCTechLargeLogRuntimeStat
}

func (e *mctechLargeLogRetriever) getFileEndTime(ctx context.Context, file *os.File) (time.Time, error) {
	var t time.Time
	var tried int
	stat, err := file.Stat()
	if err != nil {
	...
	for {
		...
		for i := len(lines) - 1; i >= 0; i-- {
			if strings.HasPrefix(lines[i], variable.MCLargeLogStartPrefixStr) {
				return ParseTime(lines[i][len(variable.MCLargeLogStartPrefixStr):])
			}
		}
		tried += len(lines)
		if tried >= maxLineNum {
...
func (e *mctechLargeLogRetriever) initializeAsyncParsing(ctx context.Context, sctx sessionctx.Context) {
	e.taskList = make(chan mctechLargeLogTask, 1)
	e.wg.Add(1)
	go e.parseDataForLargeLog(ctx, sctx)
}

func (e *mctechLargeLogRetriever) memConsume(bytes int64) {
	if e.memTracker != nil {
...
func uncompress(ctx context.Context, line string) (string, error) {
	...
	// 去掉前面的'{gzip}'和末尾的';'
	zip := line[len(variable.MCLargeLogGzipPrefixStr) : len(line)-len(variable.MCLargeLogSQLSuffixStr)]
...
type mctechLargeLogColumnValueFactory func(row []types.Datum, value string, tz *time.Location, checker *mctechLargeLogChecker) (valid bool, err error)

func getLargeLogColumnValueFactoryByName(sctx sessionctx.Context, colName string, columnIdx int) (mctechLargeLogColumnValueFactory, error) {
	switch colName {
	case variable.MCLargeLogTimeStr:
		return func(row []types.Datum, value string, tz *time.Location, checker *mctechLargeLogChecker) (bool, error) {
			t, err := ParseTime(value)
			if err != nil {
				return false, err
			}
	...
	case variable.MCLargeLogPlan:
		return func(row []types.Datum, value string, _ *time.Location, _ *mctechLargeLogChecker) (bool, error) {
			plan := parsePlan(value)
			row[columnIdx] = types.NewStringDatum(plan)
			return true, nil
		}, nil
	case execdetails.WriteKeysStr, execdetails.WriteSizeStr, execdetails.TotalKeysStr:
	...
	case variable.MCLargeLogQueryTimeStr, variable.MCLargeLogParseTimeStr,
		variable.MCLargeLogCompileTimeStr, variable.MCLargeLogRewriteTimeStr,
		variable.MCLargeLogOptimizeTimeStr, execdetails.CopTimeStr,
		execdetails.MCLargeLogProcessTimeStr, execdetails.MCLargeLogWaitTimeStr:
		return func(row []types.Datum, value string, _ *time.Location, _ *mctechLargeLogChecker) (valid bool, err error) {
	...
	case variable.MCLargeLogSuccStr:
		return func(row []types.Datum, value string, _ *time.Location, _ *mctechLargeLogChecker) (valid bool, err error) {
...
func (a *ExecStmt) SaveLargeLog(ctx context.Context, succ bool) {
	...
	enable := cfg.Metrics.LargeLog.Enabled
