...
func (e *mctechLargeQueryRetriever) getFileStartTime(ctx context.Context, file *os.File, compressed bool) (time.Time, error) {
	var t time.Time
	_, err := file.Seek(0, io.SeekStart)
	if err != nil {
		return t, err
...
func (e *memtableRetriever) setDataFromMCTechTableTTLInfos(sctx sessionctx.Context, schemas []model.CIStr) error {
	checker := privilege.GetPrivilegeManager(sctx)
	var rows [][]types.Datum
	for _, schema := range schemas {
		tables := e.is.SchemaTableInfos(schema)
